...
public class DocumentIntegrityValidator
{
    ...
    private async Task<bool> ValidateDocxFormatAsync(string filePath)
    {
        try
        {
    ...
    private async Task<DocumentStructureResult> ValidateDocumentStructureAsync(
        string originalPath, 
        string processedPath)
    {
    ...
    private async Task<string> ExtractDocumentTextAsync(string filePath)
    {
        using var document = WordprocessingDocument.Open(filePath, false);
